  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 5 }}>
        ...
        <Row icon="🔄" text={pkg.is_return ? "Gediş-dönüş" : "Birtərəfli"} />
        <Row icon="✈️" text={pkg.stops === 0 ? "Birbaşa uçuş" : `${pkg.stops} dayanacaq`} />
        <Row icon="💰" text={`${pkg.price_azn.toLocaleString()} AZN · Xidmət haqqı daxil`} bold />
      </div>
      <div style={{ padding: "8px 14px 12px", display: "flex", flexDirection: "column", gap: 7 }}>
        <a
          href={`https://wa.me/994517769632?text=${waText}`}
  ...
  return (
    <div style={{
      ...
      <div style={{ background: "linear-gradient(135deg,#16a34a,#15803d)", padding: "10px 14px" }}>
        <p style={{ margin: 0, fontSize: 11, color: "rgba(255,255,255,0.75)", textTransform: "uppercase", letterSpacing: 1 }}>
          Otel Paketi · Booking.com
        </p>
        <p style={{ margin: "2px 0 0", fontSize: 14, fontWeight: 800, color: "white" }}>{pkg.hotel_name}</p>
      </div>
      <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 5 }}>
        <Row icon="📍" text={pkg.destination} />
        <Row icon="📅" text={`${formatDate(pkg.checkin)} – ${formatDate(pkg.checkout)} (${pkg.nights} gecə)`} />
        {pkg.stars && <Row icon="⭐" text={`${pkg.stars} ulduzlu otel`} />}
        {pkg.rating && <Row icon="📊" text={`Reytinq: ${pkg.rating}/10`} />}
        <Row icon="💰" text={`${pkg.price_azn.toLocaleString()} AZN · Xidmət haqqı daxil`} bold />
      </div>
      <div style={{ padding: "8px 14px 12px", display: "flex", flexDirection: "column", gap: 7 }}>
        <a
            ...
            {msgs.map((m, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                ...
                {m.flightPackage && (
                  <FlightPackageCard
                    pkg={m.flightPackage}
                    onAlternative={() => send("Başqa uçuş göstər")}
                  />
                )}
                ...
                {m.hotelPackage && (
                  <HotelPackageCard
                    pkg={m.hotelPackage}
                    onAlternative={() => send("Başqa otel göstər")}
                  />
                )}
