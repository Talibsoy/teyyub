...
export async function POST(req: NextRequest) {
  try {
    const { destination, checkin, checkout, adults, rooms, currency } = await req.json();

    if (!destination || !checkin || !checkout) {
      return NextResponse.json({ error: "destination, checkin, checkout tələb olunur" }, { status: 400 });
    }

    const hotels = await searchHotels({ destination, checkin, checkout, adults, rooms, currency });
